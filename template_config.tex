...
\def\hfstyle {style1}              % Estilo header-footer (7 estilos)
\def\titleauthorspacing {0.5}     % Distancia entre autores [cm]
\def\titleauthormaxwidth {0.85}    % Tamaño máximo datos autores [linewidth]
\def\titlebold {true}              % Título en negrita
\def\titlestyle {style4}           % Estilo título (5 estilos)
...
\def\captionlrmarginmc {0.5}       % Margen izq/der leyenda dentro de columnas [cm]
...
\def\fontsizerefbibl {\small}      % Tamaño de fuentes al usar \bibliography{file}
...
\def\natbibrefsep {3}              % Separación entre referencia {natbib} [pt]
\def\natbibrefstyle {natsimple}    % Formato referencias natbib {apa,ieeetr,etc...}
