...
\section{Informes con \LaTeX} \label{epic}
...
\subsection{Una breve introducción} \label{epic2}
\subsubsection{epicccc} \label{epicccccc}
\subsubsubsection{epicccccccccccc} \label{key}
\paragraph{niceeeee} \label{key333}
\subparagraph{jejadsfjdsklf} \label{key3333}

	
	Este es un párrafo \ref{epic}, \ref{epic2}, \ref{epicccccc}, \ref{key}, \ref{key333}, \ref{key3333}:  puede contener múltiples \quotes{Expresiones} así como fórmulas o referencias\footnote{Las referencias se hacen utilizando la expresión \texttt{\textbackslash label}\{etiqueta\}.} como \eqref{eqn:identidad-imposible}. A continuación se muestra un ejemplo de inserción de imágenes (como la Figura \ref{img:testimage}) con el comando \href{https://latex.ppizarror.com/informe.html#hlp-imagen}{\textbackslash insertimage}:
